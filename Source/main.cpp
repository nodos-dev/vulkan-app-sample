...
struct SampleEventDelegates : nos::app::IEventDelegates
{
	...
	void HandleEvent(const nos::app::EngineEvent* event) override
	{
		...
		case EngineEventUnion::AppPinValueChanged: {
			auto pinEvent = event->event_as<nos::app::AppPinValueChanged>();
			OnPinValueChanged(*pinEvent->pin_id(), pinEvent->value()->data(), pinEvent->value()->size(), pinEvent->reset(), pinEvent->frame_number());
			break;
		}
		default:
			break;
